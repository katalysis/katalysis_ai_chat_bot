...
namespace KatalysisAiChatBot\Search\ItemList\Pager\Manager;
...
class ActionListPagerManager extends AbstractPagerManager
{
    /** 
     * @param Action $action
     * @return int 
     */
    public function getCursorStartValue($action)
    {
        return $action->getId();
    }
    ...
    public function getCursorObject($cursor)
    {
        ...
        return $entityManager->getRepository(Action::class)->findOneBy(["id" => $cursor]);
    }
    ...
    public function sortListByCursor(PagerProviderInterface $itemList, $direction)
    {
        ...
        $itemList->getQueryObject()->addOrderBy('a.id', $direction);
    }
} 